...
def write_search_results(cutoff_date, per_repo):
    ...
    expected_slugs = {path.stem for path in CANDIDATES_DIR.glob("*.yaml")}
    captured_slugs = set(per_repo)
    is_complete = captured_slugs == expected_slugs
    payload = {
        "record_status": "current" if is_complete else "historical_partial",
        "cutoff_date": cutoff_date.isoformat(),
        "repos": repos,
    }
    if not is_complete:
        payload["notes"] = (
            "Partial refresh; not evidence for complete current candidate-ledger coverage. "
            f"Missing repositories: {', '.join(sorted(expected_slugs - captured_slugs)) or 'none'}."
        )
    out = (
...
def main():
    ...
    # Merge only results from the same cutoff so partial refreshes can
    # accumulate without relabelling stale records as current evidence.
    if REFRESH_RESULTS_PATH.is_file():
        existing = yaml.safe_load(REFRESH_RESULTS_PATH.read_text(encoding="utf-8")) or {}
        existing_cutoff = existing.get("cutoff_date")
        existing_cutoff_str = (
            existing_cutoff.isoformat()
            if hasattr(existing_cutoff, "isoformat")
            else str(existing_cutoff)
        )
        if existing_cutoff_str == cutoff_date.isoformat():
